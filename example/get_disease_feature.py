...
def get_disease_feature(D_feat_path):
    # In[ ]:
    ...
    edge_init.to_csv('data/example/disease_func_edge.csv', index=False)
    dataW = pd.read_csv('data/example/disease_func_edge.csv')
    with open('data/example/Disease_func_edge.txt', 'w+', encoding = 'utf_8') as f:
    ...
    edge_file = 'data/example/Disease_func_edge.txt'
    ...
    wdeepwalk128.to_csv('data/example/disease_feature.csv', index=False)
    print('get_disease_feature done')

if __name__ == "__main__":
    D_feat_path = 'data/example/disease_I_S.csv'
