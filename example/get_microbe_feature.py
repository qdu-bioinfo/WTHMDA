...
def get_microbe_feature(M_feat_path):
    # In[ ]:
    ...
    edge_init.to_csv('data/example/microbe_func_edge.csv', index=False)
    dataW = pd.read_csv('data/example/microbe_func_edge.csv')
    with open('data/example/microbe_func_edge.txt', 'w+', encoding = 'utf_8') as f:
    ...
    edge_file = 'data/example/microbe_func_edge.txt'
    ...
    wdeepwalk128.to_csv('data/example/microbe_feature.csv', index=False)
    print('get_disease_feature done')

if __name__ == "__main__":
    M_feat_path = 'data/example/microbe_I_S.csv'
