...
def get_microbe_feature_d(M_feat_path):
    # In[ ]:
    print('get_disease_feature_d start')
    def load_weighted_Graph(fileName):
            G = nx.read_weighted_edgelist(fileName, create_using=nx.DiGraph(), nodetype=None)
            return G
    #M_feat = pd.read_csv('data/Disbiome/microbe_I_S.csv', header=None)
    ...
    edge_init.to_csv('data/Disbiome/microbe_func_edge.csv', index=False)
    dataW = pd.read_csv('data/Disbiome/microbe_func_edge.csv')
    with open('data/Disbiome/microbe_func_edge.txt', 'w+', encoding = 'utf_8') as f:
    ...
    edge_file = 'data/Disbiome/microbe_func_edge.txt'
    ...
    wdeepwalk128.to_csv('data/Disbiome/microbe_feature.csv', index=False)
    print('get_disease_feature_d done')

if __name__ == "__main__":
    M_feat_path = 'data/Disbiome/microbe_I_S.csv'
    get_microbe_feature_d(M_feat_path)
    print('get_disease_feature_d done')